...
function run()
	...
	wait(3)
	horizontall.copy = { position = vec{0, 0}, positionfollows = function(time)
			local x = time * width/2.2
			return x, (cos(time * math.pi/4 * 10) + 1) * 40 - 40
		end
	}
	...
	for i = 1, 67 do
		...
		enemy({divide1, simple, divide1}, 1, {position = vec{0, 0}, positionfollows = function(time)
			local x = time * width/10
			if x <= (width - height)/2 then
				return x, height/2

			elseif x <= (width + height)/2 then
				return x, sqrt((height/2)^2 - (x - width/2)^2) + height/2
		...
		if i == 50 then
			horizontall.copy = { position = vec{0, 0}, positionfollows = function(time)
					local x = time * width/50
					return x, (cos(time * math.pi/10 * 30) + 1) * 40 - 40
				end
			} 
