...
function draw()
	...
	if onGame() then
		...
		if state == survival then
			...
			graphics.print(string.format("%.0f", RecordsManager.getScore()), 68, 20)
			graphics.print(string.format("%.1fs", RecordsManager.getGameTime()), 68, 42)
			graphics.setFont(Base.getFont(12))
			graphics.print("Time:", 25, 48)
			graphics.print("Score:", 25, 24)
			graphics.print(string.format("Best Score: %0.f", RecordsManager.records.survival.score),     25, 68)
			graphics.print(string.format("Best Time: %.1fs", RecordsManager.records.survival.time), 25, 85)
			graphics.print(string.format("Best Mult: x%.1f", RecordsManager.records.survival.multiplier), width - 115, 83)
			...
			graphics.print(string.format("x%.1f", RecordsManager.getMultiplier()), width - 130, 35)
		elseif state == story then
			graphics.setFont(Base.getCoolFont(30))
			graphics.print(string.format("%.0f", RecordsManager.getScore()), 25, 38)
			...
			graphics.print(string.format("High Score: %.0f", RecordsManager.getStoryHighScore()), 25, 70)
